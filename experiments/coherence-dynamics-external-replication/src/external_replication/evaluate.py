...
def run_external(features_path:Path,labels_path:Path,profile_path:Path,protocol_path:Path,output:Path,iterations:int=1000)->dict[str,Any]:
    ...
    result={"schema":"coherence-dynamics.external-replication.result.v1","replication_id":protocol["replication_id"],"disposition":disp,"rows":len(frame),"success_rate":float(frame["target"].mean()),"horizon":0.75,"simple":sm,"simple_cd":em,"pr_auc_delta":delta,"roc_auc_delta":roc_delta,"task_group_bootstrap":interval,"cohorts":cohorts,"source_threshold_operating_points":{"simple":operating_metrics(frame["target"],simple,float(profile["families"]["simple"]["threshold"])),"simple_cd":operating_metrics(frame["target"],extended,float(profile["families"]["simple_cd"]["threshold"]))},"profile_sha256":sha256_file(profile_path),"protocol_sha256":sha256_file(protocol_path),"claim_boundary":protocol["claim_boundary"],"api_or_model_calls":0,"api_credit_spend_usd":0.0}
    output.mkdir(parents=True,exist_ok=True); h=write_json(output/"EXTERNAL_REPLICATION_RESULT.json",result)
    receipt={"schema":"coherence-dynamics.external-replication.run-receipt.v1","replication_id":protocol["replication_id"],"disposition":disp,"rows":len(frame),"result_sha256":h,"profile_sha256":sha256_file(profile_path),"features_sha256":sha256_file(features_path),"labels_sha256":sha256_file(labels_path),"bootstrap_iterations_requested":iterations,"model_api_calls":0,"api_credit_spend_usd":0.0}
