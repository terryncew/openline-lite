...
LOCK_ARTIFACT="olp-30pair-002-assignment-lock"
ASSIGN_JOB_NAME="assign-once"
WORKFLOW_NAME="OLP 30-pair 002 real execution — exact tag only"
WORKFLOW_PATH="olp-30pair-002-execution.yml"
...
def prior_assignment_evidence(repo:str,token:str,current_run_id:str)->list[dict]:
    ...
    while True:
        ...
        for a in arts:
            if a.get("name")==LOCK_ARTIFACT and not a.get("expired",False): evidence.append({"kind":"002_assignment_lock_artifact","artifact_id":a.get("id"),"workflow_run_id":(a.get("workflow_run") or {}).get("id")})
    ...
    while True:
        ...
        for run in runs:
            ...
            # Intentionally ignores every 001 workflow/run/artifact.
            ...
            for job in jobs:
                ...
                if job.get("conclusion")!="skipped": evidence.append({"kind":"002_prior_assign_job_attempt","workflow_run_id":run.get("id"),"job_id":job.get("id"),"job_conclusion":job.get("conclusion")})
...
def main():
    ...
    if not repo or not token or not current: raise SystemExit("002 assignment guard missing GitHub repository/token/run id")
    found=prior_assignment_evidence(repo,token,current)
    if found:
        print(json.dumps({"status":"002_REAL_ASSIGNMENT_ALREADY_ATTEMPTED","evidence":found},indent=2)); raise SystemExit(2)
    print(json.dumps({"status":"NO_PRIOR_002_REAL_ASSIGNMENT_ATTEMPT","artifact_name":LOCK_ARTIFACT,"current_run_id":current},indent=2))
