...
def parent_map() -> dict[str, dict]:
    return load_json(ROOT / "PARENT_MAP_FROZEN_002.json")["pairs"]
...
def make_invalid_record(opaque_id: str, pair_id: str, reason: str, *, client: ResponsesClient) -> dict:
    obj = {
...
def execute_common_until_fork(client: ResponsesClient, pair: dict, workspace: Path):
    ...
    while True:
        ...
        try:
            resp = client.create(instructions=pair["system_prompt"], history=history, timeout=remaining)
        ...
        if not calls:
            return {"status": "INVALID", "reason": "NO_ELIGIBLE_READ_WITHIN_FIRST_20_COMMON_PREFIX_TOOL_CALLS", "tool_count": tool_count, "elapsed": time.monotonic() - started}
        for idx, call in enumerate(calls):
            ...
            if tool_count > MAX_COMMON_PREFIX_TOOL_CALLS or tool_count > MAX_TOOL_CALLS:
                return {"status": "INVALID", "reason": "NO_ELIGIBLE_READ_WITHIN_FIRST_20_COMMON_PREFIX_TOOL_CALLS", "tool_count": tool_count - 1, "elapsed": time.monotonic() - started}
            name = call.get("name")
            args = safe_args(call)
            result = runtime.execute(name, args, max_seconds=remaining)
...
def execute_branch(*, client: ResponsesClient, pair: dict, workspace: Path, history_at_fork: list, common_tool_count: int, common_elapsed: float, eligible_call: dict, eligible_result: ToolResult, remaining_calls: list[dict], alter_eligible_result: bool):
    ...
    deadline = branch_started + active_budget
    ...
    for call in remaining_calls:
        if tool_count >= MAX_TOOL_CALLS or _remaining(deadline) <= 0:
            break
        tool_count += 1
        result = runtime.execute(call.get("name"), safe_args(call), max_seconds=_remaining(deadline))
        history.append(function_output_item(call.get("call_id"), result.output))
    ...
    while tool_count < MAX_TOOL_CALLS:
        ...
        calls = function_calls(resp)
        if not calls:
            termination = {"kind": "FINAL_AGENT_ANSWER", "tool_calls_total": tool_count, "active_seconds": min(MAX_WALL_SECONDS, common_elapsed + (time.monotonic() - branch_started))}
            break
        for call in calls:
            remaining = _remaining(deadline)
            if tool_count >= MAX_TOOL_CALLS or remaining <= 0:
                break
            tool_count += 1
            result = runtime.execute(call.get("name"), safe_args(call), max_seconds=remaining)
            history.append(function_output_item(call.get("call_id"), result.output))
            steps.append(mapper.record_completed_tool(result.observation))
        ...
        if tool_count >= MAX_TOOL_CALLS:
            termination = {"kind": "MAX_TOOL_CALLS", "tool_calls_total": tool_count, "active_seconds": min(MAX_WALL_SECONDS, common_elapsed + (time.monotonic() - branch_started))}
            break
...
def run_pair(*, pair_id: str, manifest_path: Path, sealed_zip: Path, key_path: Path, out_dir: Path):
    ...
    private = Path(tempfile.mkdtemp(prefix=f"olp002-private-{pair_id}-"))
    ...
    try:
        ...
        temp_root = Path(tempfile.mkdtemp(prefix=f"olp002-pair-{pair_id}-"))
        try:
            ...
            try:
                ...
                for oid in opaque_ids:
                    rows[oid] = make_invalid_record(oid, pair_id, "CHECKPOINT_CANNOT_RESOLVE", client=client)
                receipt = {
